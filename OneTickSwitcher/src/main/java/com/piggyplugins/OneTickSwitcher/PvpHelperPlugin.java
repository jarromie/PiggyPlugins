...
public class PvpHelperPlugin extends Plugin {
    ...
    private void focusTarget(Player player) {
        ...
        target = player;
        PiggyApiPlugin.sendClientMessage("[PVP Helper] "  + target.getName() + " focused.");
        if (config.showOverlay()) {
    ...
    private void focusTarget(MenuEntry entry) {
        if (entry.getOption().equals("Focus Target")) {
            ...
            if (targetPlayer.isEmpty()) {
                ...
                PiggyApiPlugin.sendClientMessage("[PVP Helper] Unable to focus target player");
                return;
            }
            ...
            target = null;
            PiggyApiPlugin.sendClientMessage("[PVP Helper] Target unfocused.");
            if (config.showOverlay()) {
                valueOverlay.setItemValue(0);
                valueOverlay.setHidden(true);
            }
    ...
    private final HotkeyListener spellOneListener = new HotkeyListener(() -> config.spellOneToggle()) {
        ...
        public void hotkeyPressed() {
            if (target == null) {
                clientThread.invoke(() -> {
                    PiggyApiPlugin.sendClientMessage("[PVP Helper] Focus a target before casting a spell.");
                });
            }
