...
public class BankInteraction {
    ...
    public static void withdrawX(Widget item, int amount){
        if(PiggyApiPlugin.getClient().getVarbitValue(WITHDRAW_QUANTITY)==amount){
            MousePackets.queueClickPacket();
            WidgetPackets.queueWidgetActionPacket(5,item.getId(),item.getItemId(), item.getIndex());
            return;
        }
        BankInteraction.useItem(item,"Withdraw-X");
        PiggyApiPlugin.getClient().setVarcStrValue(359,Integer.toString(amount));
        PiggyApiPlugin.getClient().setVarcIntValue(5,7);
        PiggyApiPlugin.getClient().runScript(681);
        PiggyApiPlugin.getClient().setVarbit(WITHDRAW_QUANTITY,amount);
    }
