...
public class PrayerInteraction {
    ...
    public static void togglePrayer(Prayer a){
        if(PiggyApiPlugin.getClient().getVarbitValue(a.getVarbit())==0){
            PiggyApiPlugin.getClient().setVarbit(a.getVarbit(),1);
        }else{
            PiggyApiPlugin.getClient().setVarbit(a.getVarbit(),0);
        }
    ...
    public static void setPrayerState(Prayer prayer,boolean on){
        if(PiggyApiPlugin.getClient().isPrayerActive(prayer) != on){
            togglePrayer(prayer);
        }
    }
